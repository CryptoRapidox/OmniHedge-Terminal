...
# --- 1. AYARLAR ---
st.set_page_config(page_title="OmniHedge v5.2", page_icon="🚀", layout="wide")
...
def fetch_terminal_data():
    # SourcePrices: Belirlediğin 4 borsa için özel BTC fiyat takibi
    res = {
    ...
    # 1. Binance Fiyatları (CEX Referans)
    try:
    ...
    try:
        ...
        if p_data.get("success"):
            for i in p_data.get("data", []):
                sym = i.get("symbol")
                res["Pacifica"][sym] = float(i.get("funding_rate", 0))
                if sym == "BTC":
                    res["SourcePrices"]["Pacifica"] = float(i.get("mark_price") or i.get("index_price") or 0)
    ...
    # 5. Lighter (Fiyat Verisi Eklendi)
    try:
        l_res = requests.get("https://mainnet.zklighter.elliot.ai/api/v1/markets", timeout=5).json()
        for i in l_res:
            s = i.get("symbol", "").replace("1000", "").replace("USDC", "")
            if s == "BTC":
                res["SourcePrices"]["Lighter"] = float(i.get("last_price", 0))
    except: pass
...
# BTC FİYAT MONİTÖRÜ (YENİ BÖLÜM)
st.subheader("📊 Live BTC Price Monitoring (Multi-DEX)")
p_col1, p_col2, p_col3, p_col4 = st.columns(4)
sp = data["SourcePrices"]

p_col1.metric("Pacifica BTC", f"${sp['Pacifica']:,.2f}")
p_col2.metric("Variational BTC", f"${sp['Variational']:,.2f}")
p_col3.metric("Reya BTC", f"${sp['Reya']:,.2f}")
p_col4.metric("Lighter BTC", f"${sp['Lighter']:,.2f}")
